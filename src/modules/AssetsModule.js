...
// Pehli jaanch Excel file par hoti hai (server padhta hai). Uske baad har
// "Dobara check karo" aur aakhri import /assets/import/opening/rows par jaate
// hain — wahi rows jo screen par hain, badlav ke saath — taaki ek galti ke liye
// Excel dobara na banani pade. Server har baar poori jaanch khud karta hai;
// yahan ki jaanch sirf dikhane ki hai.
//
// Sudhaar wali screen ab sabhi import ki ek hi hai (components/ImportFix.js).
// Assets sirf apni cheezein deta hai: Excel ke column jaise box, table me
// Qty/Kahan/Kiske paas, aur example row wala shortcut.
...
// hai) — wo box tabhi dikhte hain jab site ho, ya unhi par galti ho.
const impSite = (v) => impNorm(v.loc_type) === "site";
const impExt = (v) => impSite(v) && ["worker", "subcon"].includes(impNorm(v.holder_type));
const impRent = (v) => impExt(v) && impNorm(v.charge_mode) === "rent";
const impEnumOpts = (k) => () => IMP_ENUMS[k].map(([val]) => val);
const impEnumLabel = (k) => (val) => {
  const hit = IMP_ENUMS[k].find(([x]) => impNorm(x) === impNorm(val));
  return hit ? hit[1]() : val;
};
// Box ka label Excel ke column ka naam hi hai — user ko apni file me wahi
// dhoondhna hota hai. Qty ya item ka naam har row ka apna hota hai, is liye
// un par "sab rows me lagao" wala button kabhi nahi (same).
const assetImportFields = () => {
  const F = (key, extra) => ({ key, col: t("assets.xl_" + key), ...(extra || {}) });
  const pick = (key, extra) => F(key, { type: "select", options: impEnumOpts(key), optionLabel: impEnumLabel(key), same: true, ...(extra || {}) });
  return [
    F("name"),
    F("spec"),
    F("category", { type: "list", options: (L) => L.categories || [], same: true }),
    pick("tracking"),
    F("qty", { type: "number" }),
    F("unit", { type: "list", options: () => UNITS, same: true }),
    F("code", { clearable: true, showIf: (v) => impNorm(v.tracking) === "serialized" || String(v.code || "").trim() !== "" }),
    pick("condition"),
    pick("loc_type"),
    F("loc_name", { type: "select", same: true,
      options: (L, v) => (impNorm(v.loc_type) === "warehouse" ? L.warehouses : impSite(v) ? L.projects : [...(L.warehouses || []), ...(L.projects || [])]) || [] }),
    pick("holder_type", { showIf: impSite }),
    F("holder_name", { type: "select", same: true, showIf: impSite,
      options: (L, v) => ({ user: L.users, worker: L.workers, subcon: L.subcons }[impNorm(v.holder_type)] || []) }),
    F("custodian", { type: "select", same: true, showIf: impExt, options: (L) => L.users || [] }),
    pick("charge_mode", { showIf: impExt }),
    F("rent_rate", { type: "number", showIf: impRent }),
    pick("rent_basis", { showIf: impRent }),
    F("purchase_date", { type: "date" }),
    F("purchase_cost", { type: "number" }),
    F("vendor"),
    F("remarks", { wide: true }),
  ];
};
// Jagah ka type badla to purana naam/holder us type ka nahi rehta.
const assetImportAdjust = (raw, k, value, L) => {
  if (k === "loc_type") {
    const lt = impNorm(value);
    const pool = lt === "warehouse" ? L.warehouses : lt === "site" ? L.projects : null;
  ...
  if (k === "holder_type") {
    const pool = { user: L.users, worker: L.workers, subcon: L.subcons }[impNorm(value)];
...
function ImportModal({ open, onClose, onDone }) {
  ...
  useEffect(() => {
    if (open) { setStep(1); setFileName(""); setB64(""); setResult(null); setError(""); fx.reset(); }
  ...
  const check = async () => {
    ...
    setBusy(false);
    if (r && r.data && r.data.rows) { fx.take(r.data, r.message, true); setStep(2); return; }
    setError((r && r.message) || t("assets.import_failed"));
  };

  const sendRows = async (dryRun) => {
    const list = fx.payload();
  ...
  const twoLine = (a, b) => (<>
    <div>{a || "—"}</div>
    {b ? <div style={{ fontSize: 10.5, color: T.t4 }}>{b}</div> : null}
  </>);

  return (
    <Modal open={open} onClose={onClose} width={step === 2 ? 1060 : 900} title={t("assets.import_title")} sub={fileName || t("assets.import_sub")}
      footer={
        ...
        : step === 2 ? <><Btn ghost onClick={() => setStep(1)}>{t("assets.back")}</Btn>
            {fx.stale
              ...
              : <Btn onClick={() => sendRows(false)} disabled={busy || !fx.canImport}>{busy ? t("assets.importing") : t("assets.import_go", { n: fx.counts.ok })}</Btn>}</>
      ...
      {step === 2 && (
        <>
          <ImportFixPanel fx={fx} fields={assetImportFields()} adjust={assetImportAdjust}
            title={(v) => (v.name ? v.name + (v.code ? " · " + v.code : "") : "")}
            sub={(v) => [v.spec, v.category, v.tracking].filter(Boolean).join(" · ")}
            cols={[
              { head: t("assets.qty"), cell: (v) => (v.qty ? `${v.qty} ${v.unit || ""}` : "—") },
              { head: t("assets.where"), cell: (v) => twoLine(v.loc_type, v.loc_name) },
              { head: t("assets.holder"), cell: (v) => twoLine(v.holder_type, v.holder_name) },
            ]}
            rowAction={(r) => (r.status === "skipped"
              ? { label: t("assets.imp_use_example"), onClick: () => fx.setRaw(r.row, "remarks", "") }
              : null)}
            skippedHint={fx.counts.skipped ? t("assets.imp_skipped_warn", { n: fx.counts.skipped }) : ""} />
          <ErrBox>{error}</ErrBox>
        </>
      )}
      {step === 3 && result && (
