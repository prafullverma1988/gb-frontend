...
function FinanceModule(){
  ...
  // Builds a print-ready HTML invoice in a new window. Fetches the
  // full invoice (with line items) on demand, then prints. Works for
  // both ledger rows (sourceKind=customer_invoice) and the estimate
  // module preview.
  const downloadInvoicePDF = async (invoiceId, partyName) => {
    if (!invoiceId) return;
    try {
      ...
      const itemsHTML = items.length ? items.map((it, i) => `
        <tr>
          ...
          <td>${(it.description || it.clean_description || it.estimate_item_description || "—").toString().replace(/</g,"&lt;")}</td>
          <td style="text-align:center">${it.unit || "—"}</td>
          <td style="text-align:right;font-variant-numeric:tabular-nums">${parseFloat(it.qty||0).toLocaleString("en-IN")}</td>
          <td style="text-align:right;font-variant-numeric:tabular-nums">₹${(parseFloat(it.rate||0)||0).toLocaleString("en-IN")}</td>
          <td style="text-align:right;font-weight:600;font-variant-numeric:tabular-nums">₹${(parseFloat(it.this_invoice_amount||it.amount||0)||0).toLocaleString("en-IN")}</td>
      ...
      w.document.write(`<!doctype html><html><head><meta charset="utf-8"/>
        ...
        <style>
          ...
          .head{display:flex;justify-content:space-between;border-bottom:2px solid #111827;padding-bottom:14px;margin-bottom:18px}
          h1{font-size:22px;margin:0;letter-spacing:-.3px}
          .sub{font-size:12px;color:#6B7280;margin-top:4px}
          .meta{display:flex;gap:30px;margin:14px 0 18px;padding:12px 16px;background:#F8F9FB;border-radius:8px;border:1px solid #E5E7EB}
          .meta .l{font-size:10px;color:#6B7280;text-transform:uppercase;letter-spacing:.3px}
          .meta .v{font-size:13px;font-weight:700;color:#111827;margin-top:2px}
          .status{display:inline-block;padding:3px 10px;border-radius:14px;font-size:11px;font-weight:700;background:${statusColor}15;color:${statusColor};border:1px solid ${statusColor}55}
          table{width:100%;border-collapse:collapse;font-size:12px;margin-top:10px}
          th{background:#F8F9FB;color:#374151;font-weight:700;text-align:left;padding:9px 10px;border-bottom:2px solid #D1D5DB;font-size:10.5px;text-transform:uppercase;letter-spacing:.3px}
          td{padding:9px 10px;border-bottom:1px solid #F3F4F6}
          .totals{margin-top:18px;width:340px;margin-left:auto;border:1px solid #E5E7EB;border-radius:8px;overflow:hidden}
          .totals .row{display:flex;justify-content:space-between;padding:8px 14px;border-bottom:1px solid #F3F4F6;font-size:12px}
          .totals .row.net{background:#ECFDF5;border-bottom:none;font-weight:800;color:#059669;font-size:14px}
          .totals .row.ret{color:#D97706}
          .totals .row.tds{color:#DC2626}
          ...
          <div class="head">
            <div>
              <h1>TAX INVOICE</h1>
              <div class="sub">Invoice No: <strong>${inv.invoice_no||""}</strong> · ${inv.invoice_date||""}</div>
            </div>
            <div style="text-align:right">
              <span class="status">${inv.status||"Submitted"}</span>
              <div class="sub" style="margin-top:6px">${inv.source==='auto'?'🤖 Auto-Generated':inv.source==='manual'?'Manual':'Milestone-Based'}</div>
            </div>
          </div>
          <div class="meta">
            <div><div class="l">Bill To</div><div class="v">${(partyName||inv.customer_name||"—").toString().replace(/</g,'&lt;')}</div></div>
            ${inv.project_name?`<div><div class="l">Project</div><div class="v">${inv.project_name.toString().replace(/</g,'&lt;')}</div></div>`:""}
            <div><div class="l">Date</div><div class="v">${inv.invoice_date||""}</div></div>
          </div>
          <table>
            <tr>
          ...
          <div class="totals">
            <div class="row"><span>Gross Amount</span><span>₹${(parseFloat(inv.gross_amount)||0).toLocaleString("en-IN")}</span></div>
            ${parseFloat(inv.retention_amt)>0?`<div class="row ret"><span>Retention (${inv.retention_pct||0}%)</span><span>− ₹${parseFloat(inv.retention_amt).toLocaleString("en-IN")}</span></div>`:""}
            ${parseFloat(inv.tds_amt)>0?`<div class="row tds"><span>TDS (${inv.tds_pct||0}%)</span><span>− ₹${parseFloat(inv.tds_amt).toLocaleString("en-IN")}</span></div>`:""}
            ${parseFloat(inv.tax_amt)>0?`<div class="row"><span>Tax (${inv.tax_pct||0}%)</span><span>+ ₹${parseFloat(inv.tax_amt).toLocaleString("en-IN")}</span></div>`:""}
            <div class="row net"><span>NET RECEIVABLE</span><span>₹${(parseFloat(inv.net_receivable)||0).toLocaleString("en-IN")}</span></div>
          </div>
          ${inv.remark?`<p style="margin-top:18px;font-size:11.5px;color:#6B7280"><strong>Note:</strong> ${inv.remark.toString().replace(/</g,'&lt;')}</p>`:""}
          <div class="footer">Generated by GB Buildcon · ${today}</div>
        </body></html>`);
      w.document.close();
      setTimeout(() => w.print(), 400);
    } catch (e) {
      window.alert("Failed to load invoice: " + (e?.message||""));
    }
  };
...
Customer: ${partyName||""}
${ledgerRow.project?`Project: ${ledgerRow.project}\n`:""}Date: ${ledgerRow.date||""}
Net Receivable: ₹${amt}
Status: ${ledgerRow.status||"unpaid"}`;
    if (mode === "whatsapp") {
    ...
    <div style={{background:T.bg,height:"100%",display:"flex",flexDirection:"column",fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      ...
      {settlePay&&(()=>{
        ...
        const doPay=async()=>{
          ...
          }catch(e){window.alert(e?.message||"Network error");setSettleBusy(false);}
        };
        return(
          <>
