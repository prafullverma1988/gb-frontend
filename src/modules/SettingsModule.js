...
function ModuleTypeCard() {
  const [moduleType, setModuleType] = useState(null);  // null = loading
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);

  useEffect(() => {
    api.get("/settings/company").then(r => {
      if (r.success) setModuleType(r.data?.module_type || "construction");
    }).catch(() => setModuleType("construction"));
  }, []);
  ...
  const save = async (val) => {
    ...
    try {
      const res = await api.put("/settings/company", { module_type: val });
      if (res.success) {
        // Store in localStorage so ProjectsModule can read without extra API call
        localStorage.setItem("gb_company_module", val);
        setSaved(true);
  ...
  const OPTIONS = [
    {
      ...
      desc: "Residential, commercial, industrial projects. BOQ, Material, Subcon, DPR, Design.",
      icon: "🏗️",
      color: "#1565C0",
      bg: "#E3F2FD",
      bdr: "#90CAF9",
    ...
    {
      id: "solar_epc",
      label: "Solar EPC Only",
      desc: "PM Surya Ghar, BOQ kits, installation tracking, subsidy claim, serial numbers.",
      icon: "☀️",
      color: "#E65100",
      bg: "#FFF8E1",
      bdr: "#FFD54F",
    },
    {
      id: "both",
      label: "Both Modules",
      desc: "Run construction and solar EPC projects together. Each project type gets its own tabs.",
      icon: "🏗️+☀️",
      color: "#2E7D32",
      bg: "#E8F5E9",
      bdr: "#A5D6A7",
  ...
  return (
    <div style={{ background: "white", borderRadius: 10, border: `1px solid ${T.borderLight}`, overflow: "hidden", marginBottom: 20 }}>
      {/* Header */}
      <div style={{ padding: "16px 20px", borderBottom: `1px solid ${T.borderLight}` }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: T.text }}>Module Configuration</div>
        <div style={{ fontSize: 12, color: T.textLight, marginTop: 3 }}>
          Select which modules your company uses. This controls what options appear when creating projects.
        </div>
      </div>

      <div style={{ padding: "20px" }}>
        {moduleType === null ? (
          <div style={{ textAlign: "center", padding: "20px", color: T.textLight, fontSize: 12 }}>Loading...</div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {OPTIONS.map(opt => {
              const isSelected = moduleType === opt.id;
              return (
                <button key={opt.id} onClick={() => save(opt.id)}
                  style={{
                    display: "flex", alignItems: "center", gap: 14,
                    padding: "14px 16px", borderRadius: 10, cursor: "pointer", textAlign: "left",
                    border: `2px solid ${isSelected ? opt.color : T.borderLight}`,
                    background: isSelected ? opt.bg : "white",
                    transition: "all .15s", width: "100%",
                  }}
                  onMouseEnter={e => { if(!isSelected) e.currentTarget.style.borderColor = opt.bdr; }}
                  onMouseLeave={e => { if(!isSelected) e.currentTarget.style.borderColor = T.borderLight; }}>
                  {/* Radio circle */}
                  <div style={{
                  ...
                  {/* Icon */}
                  <div style={{ fontSize: 22, flexShrink: 0, lineHeight: 1 }}>{opt.icon}</div>
                  {/* Text */}
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: isSelected ? opt.color : T.text }}>{opt.label}</div>
                    <div style={{ fontSize: 11.5, color: T.textLight, marginTop: 2, lineHeight: 1.5 }}>{opt.desc}</div>
                  </div>
                  {isSelected && (
                    <div style={{ fontSize: 10, fontWeight: 700, color: opt.color, background: opt.bg, border: `1px solid ${opt.bdr}`, padding: "2px 10px", borderRadius: 20, flexShrink: 0 }}>
                      Active
                    </div>
                  )}
                </button>
        ...
        {/* Save feedback */}
        {(saving || saved) && (
          <div style={{ marginTop: 12, fontSize: 12, color: saved ? "#2E7D32" : T.textLight, display: "flex", alignItems: "center", gap: 6 }}>
            {saving ? "Saving..." : "✓ Saved — changes apply immediately for all users"}
          </div>
        )}

        <div style={{ marginTop: 14, padding: "10px 12px", background: "#F8F9FB", borderRadius: 7, border: `1px solid ${T.borderLight}`, fontSize: 11.5, color: T.textLight, lineHeight: 1.6 }}>
          ℹ️ This setting applies company-wide. Solar EPC users will only see solar project options; construction users will only see construction options.
        </div>
      </div>
    </div>
  );
