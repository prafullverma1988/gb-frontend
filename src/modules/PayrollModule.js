...
function SalarySlipModal({emp,att,month,year,onClose,paymentType,workingDays}){
  ...
  const printSlip=()=>{
    ...
    <div class="header"><h2 style="margin:0">GB Buildcon &mdash; Salary Slip</h2><p style="margin:4px 0 0">Month: ${MONTHS[month]} ${year}</p></div>
    ...
    </table><p style="margin-top:20px;font-size:10px;color:#6B7280">Generated by GB Buildcon Payroll System &middot; ${new Date().toLocaleDateString("en-IN")}</p>
    </body></html>`);
    w.document.close();
...
function SalaryLedgerTab({salaryRecords,setSalaryRecords,month,year}){
  ...
  const [payForm,setPayForm]=useState({paidDate:TODAY,paidBy:"Prafull",txRef:""});
  ...
  return(
    <div style={{maxWidth:480,margin:"0 auto"}}>
      ...
      <div style={{background:T.sb,borderRadius:12,padding:"16px 20px",marginBottom:14,textAlign:"center"}}>
        <div style={{fontSize:11,color:"rgba(255,255,255,0.5)",textTransform:"uppercase",letterSpacing:"1px",marginBottom:4}}>GB Buildcon · Site Attendance</div>
