        ...
        <div style={{ padding: "14px 18px", borderBottom: `1px solid ${T.b1}` }}>
          ...
          {data?.boq_task_count > 0 && (
            <div style={{ marginTop: 8, background: "#FFFBEB", border: "1px solid #FDE68A", borderRadius: 8,
             ...
             {t("map_plan_wizard.is_site_par")} <b>{t("map_plan_wizard.boq_task_count_kaam_pehle_se", { boq_task_count: data.boq_task_count })}</b> {t("map_plan_wizard.hain_ai_plan_boq_import_se")} <b>{t("map_plan_wizard.tenders_map_kaam_jagah")}</b> {t("map_plan_wizard.se_unhi_kaam_ko_line_pin")}
            </div>
          )}
