...
const blankSample = () => ({
  ...
  cubes_count: "3", element: "", slump_mm: "", note: "",
});
...
function NewSampleModal({ open, meta, onClose, onCreated }) {
  ...
  const save = async () => {
    ...
    const r = await rpost("/cube-samples", {
      ...
      slump_mm: v.slump_mm || null, note: v.note || null,
    });
    setBusy(false);
    ...
    <Modal open={open} onClose={onClose} width={720} title={t("rmc.new_sample")} sub={t("rmc.new_sample_sub")}
      ...
      <Grid cols={3} style={{ marginBottom: 12 }}>
        ...
        <Field label={t("rmc.grade")}
          hint={v.grade ? t("rmc.fck_is", { n: fckOf(v.grade) == null ? "—" : fckOf(v.grade) }) : ""}>
          <input style={inp} value={v.grade} disabled={fromChallan} placeholder="M25"
      ...
      <Notice>{t("rmc.sample_note")}</Notice>
      <ErrBox>{err}</ErrBox>
    </Modal>
  );
}
...
function RmcCubes({ meta, canCreate, refreshKey, onRefresh }) {
  ...
  const [fl, setFl] = useState({ project_id: "", status: "", failed: false });
  ...
  const stat = useMemo(() => ({
    total: shown.length,
    due: shown.filter((r) => r.due_7 || r.due_28).length,
  ...
  return (
    <div>
      <Notice>{t("rmc.cube_intro")}</Notice>
        ...
        {loading ? <Spinner label={t("common.loading")} />
          : shown.length === 0 ? <Empty>{t("rmc.no_samples")}</Empty> : (
            <Scroll minWidth={900}>
              ...
              {shown.map((r) => {
                const due = r.due_7 || r.due_28;
                return (
                  <Row key={r.id} cols={CC} onClick={() => setOpenId(r.id)}
                    style={r.failed ? { background: T.redL } : due ? { background: T.ambL } : undefined}>
      ...
      {open && (
        <SampleDrawer sample={open} canCreate={canCreate}
